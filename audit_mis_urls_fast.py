#!/usr/bin/env python3
"""
audit_mis_urls_fast.py
----------------------
Parallel version of audit_mis_urls.py.
Uses ThreadPoolExecutor for concurrent fetching (10 workers, no delay).
Saves a checkpoint JSON after every 100 URLs so a crash won't lose progress.

Usage:
    python3 audit_mis_urls_fast.py /path/to/file.xlsx
    python3 audit_mis_urls_fast.py /path/to/file.xlsx --resume   # resume from checkpoint
"""
...
def process_url(args):
    """Worker function: fetch and classify a single URL. Returns result dict."""
    row_idx, url = args
...
def audit(input_path: str, resume: bool = False):
    ...
    checkpoint_path = input_path.parent / (input_path.stem + "_checkpoint.json")

    print(f"\n{'='*60}")
    print(f"  IMS / MIS URL Auditor (FAST — {MAX_WORKERS} workers)")
    print(f"{'='*60}")
    ...
    NEW_HEADERS = ["Flagged", "MIS Category", "Matched Keywords", "Confidence", "Scrape Method"]
    ...
    results = dict(checkpoint)  # seed with checkpointed results
    completed = 0
    flagged_count = 0
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_job):
            ...
            status = f"FLAGGED [{result['confidence']}] — {', '.join(result['categories'])}" if result["flagged"] else "clean"
            print(f"  [{completed + len(checkpoint):>4}/{total_to_process}] {result['url'][:70]:<70} {status}")

            if result["flagged"]:
                flagged_count += 1

            # Checkpoint every N completed
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fast parallel audit of URLs for misplaced MIS product content."
    )
